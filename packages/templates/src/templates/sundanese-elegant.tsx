...
/* Mega mendung cloud motif — classic Sundanese batik border */
function MegaMendungRow({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 200 24" className="mx-auto h-6 w-full max-w-md" aria-hidden="true">
      {[0, 40, 80, 120, 160].map((x) => (
        <g key={x}>
          <path
            d={`M${x + 4} 20 Q ${x + 8} 8 ${x + 16} 10 Q ${x + 24} 2 ${x + 32} 10 Q ${x + 40} 8 ${x + 44} 20 Z`}
            fill="none"
            stroke={color}
            strokeWidth="1.2"
          />
...
export function SundaneseElegant({ data, preview }: TemplateProps) {
  ...
  const primary = theme.primaryColor ?? "#6b5b3e";
