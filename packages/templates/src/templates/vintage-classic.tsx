...
/* Ornate corner flourish (shared by hero + section dividers) */
function CornerFlourish({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 80 80" className="h-8 w-8" aria-hidden="true">
      <path
        d="M4 76 C4 30 30 4 76 4 M4 76 C20 76 76 20 76 4"
        fill="none"
        stroke={color}
        strokeWidth="1.5"
      />
...
export function VintageClassic({ data, preview }: TemplateProps) {
  ...
  const primary = theme.primaryColor ?? "#8a6d3b";
    ...
    >
      ...
      <section className="mx-auto max-w-2xl px-6 py-16">
        <div className="text-center mb-8">
          <CornerFlourish color={primary} />
          <h2 className="text-sm uppercase tracking-widest" style={{ color: primary }}>
            Rangkaian Acara
          </h2>
        </div>
          ...
          {events.map((event) => (
            <AnimateIn key={event.id} direction="up">
              <div
                ...
                style={{ backgroundColor: surface, borderColor: `${primary}44` }}
              >
