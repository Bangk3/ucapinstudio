...
const bg = "#efe6d3";
const surface = "#f7f0e2";
const text = "#4a3a2a";
const muted = "#8a7a64";

/* Twine/washi tape strip — torn paper edge feel */
function TapeStrip({ color, rotate }: { color: string; rotate: number }) {
  return (
    <div
      aria-hidden="true"
      className="mx-auto h-6 w-28 rounded-sm opacity-60"
      style={{
...
export function RusticKraft({ data, preview }: TemplateProps) {
  ...
  const primary = theme.primaryColor ?? "#8d6e4a";
    ...
    >
      ...
      <section className="relative flex min-h-dvh flex-col items-center justify-center px-6 py-20 text-center overflow-hidden">
        ...
        <div className="relative z-10 space-y-5">
          ...
          <TapeStrip color={primary} rotate={-2} />
      ...
      <section className="mx-auto max-w-2xl px-6 py-20 text-center">
        <TapeStrip color={primary} rotate={1} />
        <p className="mt-6 mb-8 text-xs uppercase tracking-widest" style={{ color: primary }}>
          Mempelai
        </p>
        ...
        <div className="space-y-6">
          {events.map((event) => (
            <AnimateIn key={event.id} direction="up">
              <div
                className="rounded-md border-2 p-6 text-center"
                style={{ borderColor: `${primary}66`, backgroundColor: surface }}
              >
      ...
      {galleryUrls && galleryUrls.length > 0 && (
        <section className="px-4 py-16">
          ...
          <GalleryLightbox
            ...
            itemClassName="aspect-square w-full rounded-md object-cover cursor-pointer"
          />
        </section>
      )}
