...
/* Gigi balang motif — traditional Betawi zigzag ornament */
function GigiBalangRow({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 200 14" className="mx-auto h-3.5 w-full max-w-md" aria-hidden="true">
      <path
        d="M0 12 L 8 2 L 16 12 L 24 2 L 32 12 L 40 2 L 48 12 L 56 2 L 64 12 L 72 2 L 80 12 L 88 2 L 96 12 L 104 2 L 112 12 L 120 2 L 128 12 L 136 2 L 144 12 L 152 2 L 160 12 L 168 2 L 176 12 L 184 2 L 192 12 L 200 2"
        fill="none"
        stroke={color}
        strokeWidth="1.6"
      />
      {[0, 16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192].map((x) => (
        <circle key={x} cx={x + 8} cy={12} r="1.4" fill={color} />
      ))}
    </svg>
  );
}

export function BetawiModern({ data, preview }: TemplateProps) {
  ...
  const primary = theme.primaryColor ?? "#b23a2a";
