...
/* Tenun stripe motif — Bugis-Makassar woven sarung band */
function TenunRow({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 200 12" className="mx-auto h-3 w-full max-w-md" aria-hidden="true">
      <rect x="0" y="0" width="200" height="12" fill={color} opacity="0.15" />
      {[0, 25, 50, 75, 100, 125, 150, 175].map((x) => (
        <g key={x}>
          <rect x={x} y="0" width="6" height="12" fill={color} opacity="0.5" />
          <rect x={x + 9} y="0" width="2" height="12" fill={color} />
        </g>
      ))}
    </svg>
  );
...
export function BugisMakassarElegant({ data, preview }: TemplateProps) {
  ...
  const primary = theme.primaryColor ?? "#2e5d4f";
