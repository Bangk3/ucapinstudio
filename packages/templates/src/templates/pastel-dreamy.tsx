...
/* Whimsical floating doodle stars + hearts (soft pastel) */
const DOODLES = [
  { top: "15%", left: "10%", char: "✦", size: "text-3xl", dur: 7, delay: 0 },
  { top: "25%", left: "85%", char: "✿", size: "text-2xl", dur: 9, delay: 1 },
  { top: "70%", left: "8%", char: "☁", size: "text-4xl", dur: 8, delay: 0.5 },
  { top: "80%", left: "88%", char: "✦", size: "text-2xl", dur: 10, delay: 2 },
  { top: "10%", left: "60%", char: "✿", size: "text-xl", dur: 11, delay: 1.5 },
] as const;

export function PastelDreamy({ data, preview }: TemplateProps) {
  ...
  const primary = theme.primaryColor ?? "#c98fb5";
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: bg,
        color: text,
        backgroundImage: "linear-gradient(135deg, #fdf6f8 0%, #f3edfb 50%, #eef7f6 100%)",
      }}
    >
      {/* Opening screen */}
      {!opened && (
        ...
        {!preview &&
          DOODLES.map((d, i) => (
            <motion.div
              ...
              key={i}
              aria-hidden="true"
              className={`pointer-events-none absolute ${d.size}`}
              style={{ top: d.top, left: d.left, color: primary, opacity: 0.5 }}
              animate={{ y: [0, -14, 0] }}
              transition={{ duration: d.dur, delay: d.delay, repeat: Number.POSITIVE_INFINITY }}
            >
              {d.char}
            </motion.div>
          ))}
      ...
      {galleryUrls && galleryUrls.length > 0 && (
        <section className="px-4 py-16">
          ...
          <GalleryLightbox
            ...
            itemClassName="aspect-square w-full rounded-2xl object-cover cursor-pointer"
          />
        </section>
      )}
