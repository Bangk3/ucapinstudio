...
/* Bismillah calligraphy ornament in an arch — luxe Islamic motif */
function CalligraphyOrnament({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 120 48" className="mx-auto h-12 w-32" aria-hidden="true">
      <path
        d="M60 44 C 24 44 8 30 8 16 C 8 6 18 2 30 6 C 38 8 44 14 60 14 C 76 14 82 8 90 6 C 102 2 112 6 112 16 C 112 30 96 44 60 44 Z"
        fill="none"
        stroke={color}
        strokeWidth="1.5"
      />
      <path
        d="M46 18 Q 60 8 74 18 Q 64 22 60 26 Q 56 22 46 18 Z"
        fill="none"
        stroke={color}
        strokeWidth="1.2"
      />
      <path d="M60 26 L 60 34 M 54 30 L 66 30" stroke={color} strokeWidth="1.2" />
      <circle cx="60" cy="38" r="2" fill={color} />
    </svg>
  );
}

export function ArabicCalligraphyLuxe({ data, preview }: TemplateProps) {
  ...
  const primary = theme.primaryColor ?? "#0f4c3a";
    ...
    >
      ...
      <section className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden px-6 py-20 text-center">
        ...
        <div className="relative z-10 w-full space-y-5">
          <CalligraphyOrnament color={primary} />
          {guestName && (
