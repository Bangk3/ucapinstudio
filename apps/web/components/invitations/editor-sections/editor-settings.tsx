...
export function EditorSettings({ invitation, tenantSlug }: Props) {
  ...
  async function saveSettings() {
    setSaving(true);
    try {
      await fetch(`/api/v1/invitations/${invitation.id}`, {
        method: "PATCH",
  ...
  async function handleDuplicate() {
    ...
    if (res.ok) {
      const { invitation: copy } = await res.json();
    ...
    return (
      <div className="flex cursor-pointer items-center justify-between gap-3 py-0.5">
        ...
        <button
          ...
          style={{ backgroundColor: checked ? "oklch(0.50 0.14 145)" : "oklch(0.88 0 0)" }}
        >
          <span
    ...
    <div className="space-y-6">
      <div className="space-y-3 rounded-lg border p-4">
        ...
        <button
          ...
          disabled={saving}
          className="w-full rounded-lg px-3 py-2 text-sm font-medium text-white disabled:opacity-60 transition-opacity"
          style={{ backgroundColor: "oklch(0.50 0.14 145)" }}
        >
